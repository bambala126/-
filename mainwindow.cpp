    ...
    , m_currentUser(user)
{
    ...
    connect(ui->lineEditQuickSearch, &QLineEdit::textChanged, this, [this](const QString& text){
        ...
        if (text.isEmpty()) {
            ...
            QModelIndex found = m_model->indexForNodeId(-1); // reset
...
void MainWindow::openTab(const NodeData& node)
{
    ...
    // --- Container with vertical splitter: content editor on top, attributes below ---
    ...
    // Attributes group
    QGroupBox*      attrGroup  = new QGroupBox(tr("Атрибуты узла"), vSplitter);
